...
export class ScenarioInput {
  ...
  render(parent) {
    ...
    const subtitle = el('p', { textContent: 'Describe an advertising scenario. AI agents will generate consumers, websites, and competing campaigns, then run real-time auctions.' });

    const textarea = el('textarea', {
      className: 'scenario-textarea',
      placeholder: 'e.g. "Two sneaker brands — Nike targeting athletes and Allbirds targeting eco-conscious millennials — competing across sports and lifestyle websites..."',
    });
    textarea.rows = 5;
    ...
    this.container.append(title, subtitle, textarea, el('div', { className: 'mt-8' }, [exampleBtn]), options, launchBtn);
  ...
  _makeSelect(label, id, values, defaultVal) {
    ...
    const select = el('select', { className: 'option-select', id: `opt-${id}` });
  ...
  _handleLaunch(textarea) {
    ...
    const numConsumers = parseInt(document.getElementById('opt-consumers').value);
    const numWebsites = parseInt(document.getElementById('opt-websites').value);
    const numCampaigns = parseInt(document.getElementById('opt-campaigns').value);

    this.onLaunch({ scenario, numConsumers, numWebsites, numCampaigns });
  }
