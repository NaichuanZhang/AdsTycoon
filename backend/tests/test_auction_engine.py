...
def _mock_campaign_agent(campaign_id, auction_id, consumer_profile, website_context, session_factory):
...
def _mock_consumer_agent(auction_id, consumer_profile, website_context, ad_description, session_factory):
    """Simulate consumer feedback."""
    db = session_factory()
    try:
        auction = db.query(Auction).filter_by(id=auction_id).first()
...
class TestRunSingleAuction:
    ...
    def test_auction_selects_highest_bid(self, mock_camp, mock_cons, db_session, db_session_factory):
        with patch("backend.services.auction_engine.SessionLocal", db_session_factory):
    ...
    @patch("backend.services.auction_engine.run_consumer_agent", side_effect=_mock_consumer_agent)
    @patch("backend.services.auction_engine.run_campaign_agent", side_effect=_mock_campaign_agent)
    def test_budget_deducted(self, mock_camp, mock_cons, db_session, db_session_factory):
        with patch("backend.services.auction_engine.SessionLocal", db_session_factory):
            sim, camp1, camp2 = _setup_simulation(db_session)
...
class TestResetSimulation:
    @patch("backend.services.auction_engine.run_consumer_agent", side_effect=_mock_consumer_agent)
    @patch("backend.services.auction_engine.run_campaign_agent", side_effect=_mock_campaign_agent)
    def test_reset_clears_auctions_and_restores_budgets(self, mock_camp, mock_cons, db_session, db_session_factory):
        with patch("backend.services.auction_engine.SessionLocal", db_session_factory):
            sim, camp1, camp2 = _setup_simulation(db_session)
