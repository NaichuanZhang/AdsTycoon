...
export class Dashboard {
  ...
  render(parent, dashboardData) {
    ...
    const title = el('h2', {
      ...
      style: 'font-family: var(--font-sans); font-size: 1.3rem; font-weight: 600; margin-bottom: 20px;',
    });

    const stats = this._renderStats(dashboardData);
    const table = this._renderCampaignTable(dashboardData.campaigns, dashboardData);
  ...
  _renderStats(data) {
    ...
    return el('div', { className: 'stats-row' }, cards.map(c =>
